...
function save(){
  ...
  c._dirty = true; // marquer pour sauvegarde cloud
  updateWC();
  // Backup local — filet de sécurité si le cloud a un conflit
  try {
    const key = `encre_backup_${P.projet_cloud_id||'local'}_ch${c.id}`;
    localStorage.setItem(key, JSON.stringify({
      contenu: c.contenu, mots: c.mots, titre: c.titre,
  ...
  } catch(e) { /* localStorage plein — pas critique */ }
}
...
function ajouterChap(){
  save();
  const newChap = {id: P.nid++, titre: 'Chapitre '+(P.chapitres.length+1), contenu:'', mots:0, niveau:2, _dirty:true, auteur_id: sbUser?.id||null};
...
function updateWC(){
  ...
  // Toujours mettre à jour ch.mots depuis le DOM — c'est la source de vérité
  // Le gonflement des stats est évité par le snapshot_debut posé par le cron
  if(P.chapitres[chapI]) P.chapitres[chapI].mots = m;
...
async function lireDocx(input){
  ...
  try{
    ...
    _baselineSession = chapitres.reduce((s,ch) => s + (ch.mots||0), 0);
    loadChap(0);
    renderPanel();
    updateSbProjet();
  ...
  if(_EDITEUR && P.projet_cloud_id){
    (async()=>{
      ...
      if(P._collab){
        ...
        // Solo : baseline = total projet maintenant
        _baselineSession = P.chapitres.reduce((s,c) => s + (c.mots||0), 0);
      }
...
async function sauvegarderCloud(){
  ...
  if(estProprietaire){
    if(_EDITEUR) save(); // Recalculer les mots seulement si on est dans l'éditeur
    const meta = JSON.parse(JSON.stringify(P));
    // Ne jamais sauvegarder le contenu textuel dans les métadonnées
  ...
  const chapsDirty = P.chapitres.filter(ch => ch._dirty !== false && ch.contenu !== undefined);
  ...
  // Pour les projets solo, mettre à jour projets.contenu.chapitres avec les mots actuels
  ...
  projets.forEach(row=>{
    ...
    if(c.stats){
      ...
      Object.entries(userStats).forEach(([d,m])=>{
        // Seulement si stats_ecriture n'a pas déjà cette date
        if(typeof m === 'number' && !statsJours[d])
...
async function getStatsSource(){
  ...
  // Fusionner les stats manuelles (jan/fév/mars etc.) — seulement si pas de vraies données ce mois
  if((!sel || sel === '_all') && window._statsManuelles){
    Object.entries(window._statsManuelles).forEach(([moisCle, total]) => {
      const prefix = moisCle.length === 7 ? moisCle : moisCle.slice(0, 7);
      const dateKey = prefix + '-01';
      const aDejaDesDonnees = Object.keys(merged).some(d => d.startsWith(prefix));
      if(!aDejaDesDonnees) merged[dateKey] = (merged[dateKey]||0) + total;
    });
  }
...
function exporterCalendrier(){
  ...
  const blocs = [
    ...
    { label: `jours écrits / ${nbJoursMois}`, val: String(nbJoursEcrits), couleur: '#4a7a5a' },
    { label: 'meilleure journée', val: meilleurJour.toLocaleString('fr-FR'), couleur: '#7a4a8a' },
  ];
  if(objectif){
    const pct = Math.min(100, Math.round(totalProjet/objectif*100));
    blocs.push({ label: 'objectif manuscrit', val: pct+'%', couleur: '#4a6a8a' });
  }
  ...
  for(let j=1; j<=nbJours; j++){
    ...
    const v = stats[j]||0;
    const isToday = j===now.getDate() && _calOffset===0;
...
async function renderGreeting(){
  ...
  if(sbUser){
    const { data: statsToday } = await sb.from('stats_ecriture')
      ...
      .eq('user_id', uid)
      .eq('date', today);
    motsAujourdhui = (statsToday||[]).reduce((s,r)=>s+r.mots,0);
  }
...
function exporterCommentaires(){
  ...
  const lignes = _commDashData.map(c => {
    const date = new Date(c.cree_le).toLocaleDateString('fr-CA');
    const ctx = c.contexte ? `« ${c.contexte} »\n` : '';
    const parent = c.parent_id ? '  ↩ Réponse\n' : '';
    return `${parent}[${date}] ${c.auteur}\n${ctx}${c.texte}\n${'─'.repeat(40)}`;
  }).join('\n');
...
function copierLienLecture(projetId, e){
  e?.stopPropagation();
...
async function archiverProjet(id, e){
  ...
  const row = dbProjets.find(r => r.id === id);
  if(!row) return;
  const garder = confirm(`Archiver "${row.nom}" ?\n\nOK = garder le contenu des chapitres\nAnnuler = vider le contenu (garder les stats)`);
